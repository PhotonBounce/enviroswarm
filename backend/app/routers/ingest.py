...
async def ingest(
    ...
    # For MVP we just count existing readings today and compare against tier.
    today_start = datetime.now(timezone.utc).replace(hour=0, minute=0, second=0, microsecond=0)
    # Count all readings today for this user's stations
    count_result = await db.execute(
        select(SensorReading).join(SensorStation).where(
